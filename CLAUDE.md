...
| `STOCKX_API_KEY` / `STOCKX_CLIENT_ID` / `STOCKX_CLIENT_SECRET` / `STOCKX_REFRESH_TOKEN` | StockX real-time pricing. Without all four, `backend/app/ingestion/stockx.py`'s loop never starts. Needs an approved developer.stockx.com application (manual review, not guaranteed, ~1-2 weeks) plus a one-time interactive OAuth2 authorization — see "StockX setup" below for the exact walkthrough. | No (yet) |
| `STOCKX_INGEST_INTERVAL_SECONDS` | Override the StockX ingestion poll interval (default 3600s / 1h). | No |
...
`backend/app/ingestion/stockx.py` is a fourth connector and different in kind from the other three: instead of feed cards, it writes real `price_snapshots` rows for every figure in the catalog — which is exactly what `pricing/service.py` already reads from, so once it's live the price charts, confidence bands, collection valuation, and wishlist alerts all automatically switch from static seed data to real market data with no other code changes. Also **not yet exercised against the live API** (StockX's real API reference is login-gated behind an approved developer account, so this was built from a well-documented third-party open-source client rather than official docs — see the module docstring). Treat the first real run as a verification pass.

X/Twitter, TikTok, Xiaohongshu, Weibo, and popmart.com's own website all remain deliberately unbuilt: X killed its free API tier on 2026-02-06 (now pay-per-use, $0.005/read, no free option); TikTok has no general public search/discovery API; Xiaohongshu and Weibo are enterprise-partnership-only with no self-serve foreign-developer access; popmart.com's `/news` page (and even its `sitemap.xml`) is a fully client-rendered Next.js app with no server-rendered content or accessible API to fetch — real product announcements from Pop Mart's own voice already come through the YouTube official-channel connector instead. Scraping around any ToS-restricted platform is a deliberate non-goal, not an oversight (same reasoning already applied to Reddit above).
...
### StockX setup (for real-time pricing)

1. Apply at [developer.stockx.com](https://developer.stockx.com) — describe the app as a live Pop Mart collector platform and link the beta URL (`https://beta-backend-beta-0e8e.up.railway.app`) so the review team has something real to check. Manual review, commonly 1-2 weeks, **not guaranteed** (StockX has declined applicants citing "lack of content").
2. Once approved, create an "Application" in the developer portal's Keys page. This gives you an **API key** (`x-api-key`) and lets you configure a **Client** with a `client_id`/`client_secret` and a redirect URI (any URL works — e.g. the beta URL — you just need to read the `code` param back out of the resulting redirect, even if that page 404s).
3. StockX uses OAuth2 Authorization Code flow (via Auth0, `accounts.stockx.com`) — this needs a one-time interactive login with a real StockX account granting consent, requesting the `offline_access openid` scopes (the portal will show you the exact authorize URL for your registered app). Open it, log in, approve, and copy the `code=` value from the redirect URL's query string.
4. Give me the API key, client_id, client_secret, redirect URI you registered, and that authorization code — I'll do the one-time code-for-tokens exchange and keep only the resulting `refresh_token` (the code itself is single-use and expires quickly). That refresh_token plus the API key/client_id/client_secret are what get set as the four `STOCKX_*` Railway variables — the connector refreshes its own access token automatically from then on, no further interaction needed.
